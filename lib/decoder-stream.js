...
DecoderStream.prototype.removeListener = function (ev, fn) {
  if ('packet' == ev) {
    debug(
      'removeListener(): remapping "packet" event listener as "data" event listener'
    );
    ev = 'data';
  }
...
DecoderStream.prototype.pagein = function (page, packets, fn) {
  ...
  function afterPacketout(rtn, bytes, b_o_s, e_o_s, granulepos, packetno) {
    ...
    if (1 === rtn) {
      ...
      var p = Buffer.from(packet.packet);
      packet.packet = p;

      if (b_o_s) {
        self.emit('bos');
      }
      p._callback = afterPacketRead;
